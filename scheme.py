...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('Number of values and parameters do not match')
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('Mismatch number of args')
        # END PROBLEM 4
...
def make_let_frame(bindings, env):
    ...
    symbols = bindings.map((lambda binding: binding.first))
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        ...
        check_formals(formals)
        env.define(target.first, MacroProcedure(formals, expressions.second, env))
        return target.first
    raise SchemeError('Error in do_define_macro')
    # END Problem 21
